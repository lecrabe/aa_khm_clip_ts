...
for(data in data_input){
  system(sprintf("drive  pull %s",
                 paste0("Image_Time_series/",data)))
}

#### Create two destination folders
dir.create(paste0(sae_dir,"landsat/"), recursive = T,showWarnings = F)
dir.create(paste0(sae_dir,"sentinel/"),recursive = T,showWarnings = F)
...
lapply(lsat,function(x){file.rename(paste0("Image_Time_series/",x),paste0(sae_dir,"landsat/",x))})
lapply(stnl,function(x){file.rename(paste0("Image_Time_series/",x),paste0(sae_dir,"sentinel/",x))})
